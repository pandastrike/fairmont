...
We're ready now to define the function that will watch the files and run our tests when any of them change. To define it, we're going to curry `each` with a function that take a single pathname. We'll define a flow based on that pathname.
...
Node's FSWatcher sometimes generates duplicate events. We also don't really need to keep running tests if we're making a bunch of changes in rapid succession. So we `throttle` the events to ignore events that occurr within 5 seconds of the last event we processed.

        throttle 5000

We get two arguments from the events. These are passed in as an array. We'll log that we got an event and then run `npm test`.

        map ([event, file]) ->
          console.log "Detected change in #{file}"
          shell "npm test"

We log the output to standard out.
